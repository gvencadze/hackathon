...
import MonthlyDonate from "./panels/monthlyDonate";
import MonthlyDonatePostingSetup from "./panels/monthlyDonatePostingSetup";
import MonthlyDonateNewsFeedSnippetHeader from './panels/monthlyDonateNewsFeedSnippetHeader';
import MonthlyDonateDetailView from "./panels/monthlyDonateDetailView";
	...
	render() {
		return (
			...
			>
				...
				<Panel id="monthlyDonatePostingSetup">
					<PanelHeader left={<PanelHeaderBack onClick={() => {this.setState({activePanel: 'monthlyDonate'})}}/>}>Konstantin Gvencadze</PanelHeader>
					<MonthlyDonatePostingSetup/>

					<Div>
						<Div>
							<Button size="xl" mode="primary" onClick={() => {this.setState({activePanel: 'monthlyNewsFeedSnippet'})}}>Поделиться</Button>
						</Div>
					</Div>
				</Panel>

				<Panel id="monthlyNewsFeedSnippet">
					<MonthlyDonateNewsFeedSnippetHeader/>
							...
							<Card size="l">
								...
								<Div>
									...
									<Text style={{
										...
										Konstantin Gvencadze · Помощь нужна каждый месяц
									</Text>
								</Div>
								...
								<Div style={{
									...
									<InfoRow header="Собрано в сентябре 8 750 ₽">
										<Progress value={87}/>
									</InfoRow>
									<Button size="m" mode="primary" onClick={() => {this.setState({activePanel: 'targetDonateDetailView'})}}>Помочь</Button>
								</Div>
